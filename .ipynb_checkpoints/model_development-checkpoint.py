...
x_cols = [x for x in df.columns if x != 'CustomerId']
dataframe_check(df[x_cols])
...
cat_cols, num_cols, cat_but_car =grab_col_names(df[x_cols])
...
ohe_cols = [col for col in df.columns if df[col].nunique() > 2 and df[col].dtype not in [int, float]]
ohe_cols
df=one_hot_encoder(df, ohe_cols)

df.head()
def Robust_Scaler(dataframe,col):
    rb=RobustScaler()
    dataframe[col] = rb.fit_transform(dataframe[[col]])
...
experiment_name = "Churn_Modellingggg"
mlflow.set_experiment(experiment_name)

registered_model_name = "ChurnRFModel"
